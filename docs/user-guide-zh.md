...
当前版本仍需继续完善收藏夹/系列选择与失败重试、B站受限内容诊断、任务历史与恢复、输出文件快捷打开和长扫描 PDF 进度。正式安装包之前会先实现应用托管运行环境，使新 Mac 无需手工安装 conda/Homebrew；LLM/OCR 仍由用户配置 API，ASR 模型由应用按需管理。完整优先级和验收标准见 [`docs/todo.md`](todo.md)。

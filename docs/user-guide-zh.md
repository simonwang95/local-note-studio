...
批量日志分成两个阶段：`[抓取 i/n] 草稿已准备` 只表示动态正文和图片已经进入临时草稿区，尚未改动正式笔记；随后程序先进行完整性比对，仅对新增或不完整笔记调用 Qwen。已有完整笔记会逐篇显示“无需更新（未调用 Qwen）”。如果整批都已存在，最终会明确显示检查数量、无需更新数量、模型调用 `0` 次和“未新增、未改动文件”，这是正常的 `no_changes` 结果，不是完整性警告。多篇连续调用 Qwen 时，日志每 10 秒显示一次冷却剩余时间；如果下一篇已经存在完整笔记，会立即跳过，不会先等待冷却。

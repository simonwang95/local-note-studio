...
“预览命令”只显示两阶段流程，其中 `<converted-markdown-path>` 是占位符，不会执行抓取或 Qwen。请点击“运行任务”开始正式处理。正式运行时，转换草稿只写入系统临时目录；Qwen 成功后才会把正式 Markdown 和图片资产写入“本次输出目录”，最终笔记不会包含已删除的临时草稿路径。

...
“本次输出目录”和“输入源 URL、文件路径或目录路径”会分别保存最近使用记录。选择路径、手动输入后离开输入框、预览命令或运行任务时都会更新对应列表；两个列表互不覆盖，最多各保留 12 条，仅保存在本机界面的 `localStorage` 中，不属于任务历史，也不会写入 Manifest。每个列表都可以删除当前输入框对应的记录，也可以清空整组最近记录；这些操作只影响界面候选记录，不会删除任何输出文件或任务历史。隐身模式开启时不会新增最近输入/输出记录。
...
如不希望本次任务读取或更新 Manifest、B站已处理列表等增量记录，可勾选“隐身模式”。隐身模式仍会生成笔记，并保留“任务历史与恢复”中的本地历史；它只关闭 source/video/quickread Manifest、关键帧 Manifest、B站已处理列表、转录报告、批量失败状态和最近输入/输出记录写入。由于“只重试失败项”依赖批量失败状态，因此不能与隐身模式同时使用。

“输出文件名（可选）”只建议用于单个文件、单个 URL 或单个目录导出任务。留空时程序保持默认命名；填写时可以省略 `.md` 或 `.epub` 后缀。文件名不能包含目录分隔符，目录批量视频任务不支持自定义同一个输出名，避免多个视频写到同一个文件。

勾选“文件名补充日期”后，新整理笔记的文件名会以发布时间开头，例如 `2026-07-14-BILI-OPUS-标题_动态ID.md`；已带日期前缀的文件不会被重复添加，缺少可解析发布日期的文件会被跳过并在结果中报告。界面上另有独立的“补充文件名日期”按钮，可对已整理好的笔记目录批量补全同样的日期前缀（幂等，重复运行安全）。

勾选“启用思考”后，通用笔记整理和视频摘要/思维导图会启用模型思考（reasoning），通常会增加生成时间。生成后的思维导图会统一为每层 2 空格的 Markdown 列表；通用整理若仍是平铺或没有形成“主题 → 子主题 → 具体要点”三级结构，会依据完整笔记单独重建导图并再次校验；视频整理则保留占位符供定向重试。两条链路都不会把不合格导图静默当成完成。
...
其中 `结构化正文` 是主笔记，`校对正文` 放在正文类内容之后，`原始字幕` 放在最后。新配置默认不保留原始字幕；如果确实需要完整转写，可勾选“保留原始字幕”。关闭时 Qwen 仍会用字幕完成整理和校对，完整整理后的最终 Markdown 不保留原始字幕块。若模型失败或仍有 AI 占位符，失败中间态会暂时保留转写以便重复同一来源时只重试整理；补齐成功后再自动删除。

如果视频属于访谈、问答或多人讨论，可以勾选“对话检测与角色标注”。程序会先用 Qwen 判断内容是否具有多人轮流发言特征；确认是对话后，在校对正文中尽量标注主持人、嘉宾或“说话人 A/B”。该选项会增加一次模型调用，普通单人讲解建议保持关闭。

如果勾选“A股术语校验”，Qwen 在整理和校对时会尽量保留股票名称/代码，并在结果里附带 `## A股术语校验` 小节。动态图文整理不依赖此选项，可保持关闭；即使开启，证券代码清理也只处理正文行内空格，不会改变思维导图的行首缩进。写入前还会再次检查导图是否保持“主题 → 子主题 → 具体要点”三级结构。
...
对于长图文，B站动态详情接口有时只返回以 `...` 结尾的摘要，并省略正文内图片。程序会在接口完成权限、作者和发布时间校验后，继续读取同一登录态下服务端页面中的完整图文节点，保留原有标题层级和图片位置；页面读取暂时失败时才退回接口内容。转换草稿会记录本地化原文的 SHA-256，Qwen 整理完成后再次逐字校验，因此只有“存在 `## 原文抽取` 标题”但正文被截断的文件不会再被判定为完整。

如果日志显示“B站接口返回权限占位”，说明接口本身没有返回正文。日志里会包含 B站返回的解锁提示和档位，例如“168元充电”。这时需要确认导出 cookie 的浏览器账号已经加入对应 UP 主的对应充电档位；仅登录 B站账号还不够。

正文很短的图文动态（默认少于 1000 字）不会调用 Qwen 整理，而是直接保留原始图文，笔记会打上 `organized/verbatim` 标签并在“原文保留”小节说明原因。这个行为由 `env.local` 里两个开关控制：`QWEN_ORGANIZE_SHORT_OPUS_SKIP`（默认 `true`，设为 `false` 可让所有图文都走 Qwen）和 `QWEN_ORGANIZE_SHORT_OPUS_MAX_CHARS`（默认 `1000`，设为 `0` 可关闭跳过逻辑）。两者都不在界面里配置，需要编辑 `env.local`（可参考 `worker/env.example`）。
...
批量日志分成两个阶段：空间分页阶段显示“收集到候选图文”，表示本轮选中的待检查项目，不代表新增了正式笔记；`[抓取 i/n] 草稿已准备` 只表示动态正文和图片已经进入临时草稿区，尚未改动正式笔记。随后程序先进行完整性比对，仅对新增或不完整笔记调用 Qwen。已有完整笔记会逐篇显示“无需更新（未调用 Qwen）”。如果整批都已存在，最终会明确显示检查数量、无需更新数量、模型调用 `0` 次和“未新增、未改动文件”，这是正常的 `no_changes` 结果，不是完整性警告。全部整理成功或确认无需更新后，日志还会确认临时草稿与恢复点已经清理。多篇连续调用 Qwen 时，日志每 10 秒显示一次冷却剩余时间；如果下一篇已经存在完整笔记，会立即跳过，不会先等待冷却。
...
本地视频/音频使用和 B站单链接相同的笔记结构，也支持“关键帧图文笔记”和可选的“保留原始字幕”。如果媒体文件旁边已有同名 `.srt`，可以选择“同目录 SRT 字幕优先”；如果字幕质量差，可以切换为“ASR 语音转写优先”。关闭“保留原始字幕”不会删除旁边已有的 `.srt` 源文件。

重复处理单个文件或目录时，已有同名笔记会显示“无需重复转录（未调用 ASR）”。目录批量结果会分别统计“新建/更新、无需更新、失败”；如果全部已有，会明确显示 ASR/Qwen 调用 `0` 次和正式文件保持不变。B站收藏夹/系列、UP 主视频同步以及文档/网页批量使用同一套最终状态：已有完整结果与“因批量上限、增量状态或失败重试策略跳过”会分开说明，不再统一显示成模糊的“成功”或固定 `skipped: 1`。

...
Local Note Studio 0.1.25 提供受限 Agent CLI、本地 stdio MCP Server 和已有 Markdown 笔记的只读检索。OpenHanako 等 Agent 只负责选择命名 Profile、触发任务和查询状态；采集、转写、Qwen 整理、Manifest、恢复点和完整性检查仍由 `worker/local_note_studio_worker.py` 及现有脚本完成。
...
A股确定性证券参考表只用于校验，不授权 Qwen 为原文没有代码的股票补代码。写文件前会检查 Qwen 新增的 6 位代码：原文没有该代码时从整理区移除；原文已有代码时按证券参考表确定性纠正 `.SH`/`.SZ` 后缀。清理仅折叠正文行内的冗余空格，必须保留 Markdown 行首缩进；清理完成后再次校验思维导图层级，避免后处理把合格导图压平。原文抽取保持不变，文件末尾的“A股术语校验”表仍由 Python 生成。

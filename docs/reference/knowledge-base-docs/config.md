...
| `DEFAULT_LLM_PROVIDER` | `mtplx` | 默认本地模型服务 |
| `DEFAULT_LLM_API_BASE` | `http://127.0.0.1:8000/v1` | OpenAI-compatible API 地址 |
| `DEFAULT_LLM_MODEL` | `mtplx-qwen38-27b-optimized-speed` | 默认 Qwen3.8 27B 整理模型 |
...
| `QWEN_ORGANIZE_MAX_CHARS` | `60000` | 草稿正式整理时的单块最大字符数 |
| `QWEN_ORGANIZE_OVERLAP_CHARS` | `800` | 草稿正式整理和综合整理时的重叠上下文字符数 |
| `QWEN_ORGANIZE_SYNTHESIS_MAX_CHARS` | `80000` | 多分块综合整理的最大字符数 |
...
| `SUMMARY_API_URL` | `http://127.0.0.1:8000/v1` | B 站后处理使用的 OpenAI-compatible API |
| `SUMMARY_MODEL` | `mtplx-qwen38-27b-optimized-speed` | B 站摘要、导图、校对模型 |

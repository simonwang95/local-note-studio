...
- Released the source tree as `0.1.18` after closing the three `0.1.17` visual-acceptance blockers. Bilibili attachment OCR/vision now sends an explicit `4096`-token default budget with an `8192` hard cap and a separate `300`-second timeout, records safe finish/token/cache diagnostics, ignores pre-Schema-2.0/rules-version caches, and preserves the post plus images for retry on truncation, empty/invalid output, timeout, cancellation, or temporary service failure.
- Completed fresh-cache OpenHanako-compatible stdio MCP acceptance in `/private/tmp/local-note-opus-018.T5FQEw` while only reading the existing Application Support Cookie/model configuration. Opus `1227413653675835411` completed two low-relevance fan-ranking images in 87 seconds with 3,178 total completion tokens, no failures or financial image expansion, and preserved `2026-07-21T15:11:51+08:00`; its immediate rerun returned `no_changes`, `cache_hits=2`, and `model_calls=0`. Opus `1215072468872462337` completed its high-relevance 72-company image in 134 seconds with 2,056 completion tokens, retained ambiguous details as pending verification, preserved `2026-06-18T09:01:45+08:00`, and produced the deterministic `.SZ` suffixes for the four regression stocks; its immediate rerun returned `no_changes`, `cache_hits=1`, and `model_calls=0`.
- Verified the MCP status history with `lock=null`, numeric `completion_tokens`/`max_tokens` diagnostics intact, credential/browser-profile/image-base64 fields omitted, and no residual MCP, Worker, converter, or organizer process after EOF. The existing LM Studio worker was the only matching long-lived process and was not modified.
- With Node.js `20.20.2` and `course-whisper` Python `3.11.15`, the final suite passes the frontend build/compatibility check, 94 Python tests, and 10 Rust tests. Built `Local Note Studio_0.1.18_aarch64.dmg`, verified its checksum and read-only mount, strict deep ad-hoc signature, arm64 executable, `0.1.18` Bundle/Worker versions, bundled visual/time/stock fixes, required Worker/Agent/MCP resources, and absence of `env.local`. The 3,464,315-byte artifact SHA-256 is `cc15118bb6a0d9e502df5386d7b412e25cca30e69824737e6c96cef2336fc31f`.
